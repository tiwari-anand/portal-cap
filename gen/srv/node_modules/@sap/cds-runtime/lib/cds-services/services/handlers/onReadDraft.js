...
const _allInactive = (req, columns, model) => {
  ...
  const pk = Object.keys(model.definitions[req.target.name.replace(/_texts$/, '')].keys).find(
    ele => ele !== 'IsActiveEntity'
  )
...
const _allActive = (req, columns) => {
  ...
  _buildWhere(req.query, table)
  _buildOrderBy(req.query, cqn.SELECT.columns, table)

  return { cqn: getEnrichedCQN(cqn, req.query.SELECT, req.query.SELECT.where), scenario: 'ALL_ACTIVE' }
}
...
const _activeWithoutDraft = (req, draftWhere, columns) => {
  ...
  let cqn = SELECT.from(active.table)
...
const _getUnionCQN = (req, draftName, columns, subSelect, draftWhere) => {
  ...
  const activeDocs = getEnrichedCQN(SELECT.from(req.target), req.query.SELECT, draftActiveWhere, false)
...
const _generateCQN = (req, columns, serviceName, model) => {
  // TODO: How to get from req? Maybe with new CSN
  if (req._.odataReq.getUrlObject().path.endsWith('SiblingEntity')) {
...
const _handler = async function (req) {
  ...
  const reqClone = Object.assign({}, req, { query: {} })
  reqClone.query = _copyCQNPartial(sqlQuery)

  if (req.query._streaming) {
    adaptStreamCQN(reqClone.query)
    reqClone.query._streaming = true
    return req.run(reqClone.query)
  }

  const cqnScenario = _generateCQN(reqClone, _getColumns(reqClone, this.model), this.name, this.model)
  ...
  const result = await req.run(cqnScenario.cqn)
