...
const _getUpdateDraftCQN = ({ query, target: { name } }, singleKey) => {
  ...
  return UPDATE(ensureDraftsSuffix(name))
    .data(set)
...
const _handler = async function (req) {
  ...
  let result = await req.run(_getSelectCQN(this.model, req, singleKey))
  ...
  await Promise.all([req.run(updateDraftCQN), req.run(updateDraftAdminCQN)])

  result = await req.run(_getSelectCQN(this.model, req, singleKey, false))
  if (result.length === 0) {
    req.reject(404)
  }
