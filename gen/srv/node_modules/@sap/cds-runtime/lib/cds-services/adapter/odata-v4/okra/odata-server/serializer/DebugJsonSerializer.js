...
class DebugJsonSerializer {
  ...
  _buildPathSegments (segments) {
    return segments.map(segment => /* NOSONAR */ {
      let result = {
  ...
  _buildApplyQueryOption (apply) {
    return apply.map(transformation => {
      let result = {
        transformation: Object.keys(TransformationKind).find(
          name => TransformationKind[name] === transformation.getKind()
        )
      }
      ...
      switch (transformation.getKind()) {
        ...
        case TransformationKind.BOTTOM_TOP:
          result.method = Object.keys(BottomTopMethod).find(
            name => BottomTopMethod[name] === transformation.getMethod()
          )
        ...
        case TransformationKind.CUSTOM_FUNCTION:
          result.function = transformation
            .getFunction()
            .getFullQualifiedName()
            .toString()
  ...
  _buildAggregateExpression (expression) {
    ...
    if (expression.getStandardMethod() !== null) {
      result.standardMethod = Object.keys(AggregateMethod).find(
        name => AggregateMethod[name] === expression.getStandardMethod()
      )
    }
    ...
    if (expression.getInlineAggregateExpression()) {
      result.inlineAggregateExpression = this._buildAggregateExpression(expression.getInlineAggregateExpression())
    }
    return result
  }
