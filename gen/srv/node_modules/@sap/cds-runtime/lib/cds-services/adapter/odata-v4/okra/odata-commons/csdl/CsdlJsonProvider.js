...
class AnnotationParser {
  ...
  parse (element, annotationBuilder, annotations = []) /* NOSONAR */ {
...
class CsdlJsonProvider extends CsdlProvider {
  ...
  constructor (edmJson) {
     ...
     * @private
     */
    for (const key of Object.keys(edmJson).filter(k => !k.startsWith('$'))) this._containerJson = edmJson[key]
   ...
   * @override
   */
  getEntitySet (entityContainerName, entitySetName) /* NOSONAR */ {
   ...
   * @override
   */
  getSingleton (entityContainerName, singletonName) /* NOSONAR */ {
    const containerData = this._getPropertyValue(this._edmJson, entityContainerName.namespace, entityContainerName.name)
    if (!containerData) return null
   ...
   * @private
   */
  _readFacets (obj, objJson, allowedFacets) /* NOSONAR */ {
