...
class CommandExecutor {
   ...
   */
  execute (successCommands, failCommands, initialError, endCallback) /* NOSONAR */ {
    ...
    const chainEndCallback = error => {
      if (error) {
        ...
        if (statusCode >= 400 && statusCode < 500) {
          this._logger.warning(error.message)
        } else {
          this._logger.error(error)
        }

        if (failCommands) {
          this._logger.path('Executing fail command chain...')
          this._error = error
   ...
   * @private
   */
  _execute (commands = [], index = 0, callback) /* NOSONAR */ {
