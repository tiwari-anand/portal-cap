...
const _getParamData = parameters => {
  ...
  for (const segmentParam of parameters) {
    ...
    const value = typeName.startsWith('Int') ? Number(segmentParam.getText()) : segmentParam.getText()
    ...
    paramData[edmRef.getName()] = value
  }
...
function _entityOrTypeName (navSourceSegment, entityOrTypeName) {
...
const _addForeignKeys = (service, req, data) => {
  ...
  if (navProperty.getPartner() && navProperty.getPartner().getReferentialConstraints().size) {
    ...
    const entityOrTypeName = _entityOrTypeName(navSourceSegment)

    const navigationName = navProperty.getName()
    const navigationDefinition = service.model.definitions[entityOrTypeName].elements[navigationName]
...
/**
 * Get a deep copy of the request payload (preserves the original payload).
