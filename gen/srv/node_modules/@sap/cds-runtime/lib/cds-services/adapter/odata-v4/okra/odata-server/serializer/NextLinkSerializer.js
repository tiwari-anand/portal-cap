...
class NextLinkSerializer {
  ...
  _removeSkiptoken (queryOptionsString) {
    ...
    const newQueryOptions = queryOptionsString
      .substring(1)
      .split('&')
      .filter(queryOption => !queryOption.startsWith(QueryOptions.SKIPTOKEN))
      .join('&')
      ...
      switch (name) {
        ...
        case QueryOptions.ORDERBY:
          serializedValue = value
            .map(item => this._serializeExpression(item.getExpression()) + (item.isDescending() ? ' desc' : ''))
            .join(',')
          break
  ...
  _serializeSelect (selectItem) {
    ...
    return selectItem.isAllOperationsInSchema()
      ...
      : selectItem
        .getPathSegments()
        .map(segment => encodeURIComponent(segment.getPathSegmentIdentifier()))
        .join('/')
  }
  ...
  _serializeExpand (expandItem) {
    ...
    return (
      expandItem
        .getPathSegments()
        .map(this._serializePathSegment, this)
        .join('/') + (options.length ? '(' + options.join(';') + ')' : '')
    )
  }
    ...
    switch (expression.getKind()) {
      ...
      case ExpressionKind.UNARY: {
        ...
        return (
          ...
          (expression.getOperator() === 'not' ? ' ' : '') +
          (expression.getOperand().getKind() === ExpressionKind.BINARY ||
          expression.getOperand().getKind() === ExpressionKind.ALIAS
            ? '(' + operand + ')'
            : operand)
        )
      }
      ...
      case ExpressionKind.BINARY: {
        ...
        return (
          (expression.getLeftOperand().getKind() === ExpressionKind.BINARY ||
          expression.getLeftOperand().getKind() === ExpressionKind.ALIAS
            ? '(' + left + ')'
            : left) +
          ' ' +
          expression.getOperator() +
          ...
          expression.getRightOperand().getKind() === ExpressionKind.ALIAS
            ? '(' + right + ')'
            : right)
        )
      }
      ...
      case ExpressionKind.METHOD:
        return (
          ...
          expression
            .getParameters()
            .map(this._serializeExpression, this)
            .join(',') +
          ')'
        )
  ...
  _serializeSearch (expression) {
    if (expression.getKind() === ExpressionKind.UNARY) {
      ...
      return (
        (expression.getLeftOperand().getKind() === ExpressionKind.BINARY ? '(' + left + ')' : left) +
        ' ' +
   ...
   * @private
   */
  _serializePathSegment (segment) /* NOSONAR */ {
    ...
    if (
      kind === ResourceKind.FUNCTION_IMPORT ||
      kind === ResourceKind.BOUND_FUNCTION ||
      kind === ResourceKind.UNBOUND_FUNCTION
    ) {
      result +=
        '(' +
        segment
