...
class CommandFactory {
  ...
  static createForSingleRequest (options) {
    ...
    const uriParser = options.componentManager
      .getComponent(Components.URI_PARSER)
      .setPerformanceMonitor(context.getPerformanceMonitor())
    const contentNegotiator = options.componentManager.getComponent(Components.CONTENT_NEGOTIATOR).setLogger(logger)
    const localeNegotiator = options.componentManager.getComponent(Components.LOCALE_NEGOTIATOR)
    const requestValidator = options.componentManager.getComponent(Components.REQUEST_VALIDATOR).setLogger(logger)
    const dispatcher = options.componentManager
      .getComponent(Components.DISPATCHER)
      .setLogger(logger)
      .setPerformanceMonitor(context.getPerformanceMonitor())
    ...
    let result = [
      ...
      [new QueryOptionsValidationCommand(requestValidator, request, response, logger), 'Query options validation'],
      ...
      [new SetResponseHeadersCommand(request, response, service.getFormatManager(), logger), 'Set response headers'],
      [new SetStatuscodeCommand(request, response, HttpStatusCode.resolveSuccessStatusCode, logger), 'Set status code'],
