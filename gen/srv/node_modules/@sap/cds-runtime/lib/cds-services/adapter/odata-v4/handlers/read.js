...
const { removeContainmentKeys, resolveStructuredName } = require('../utils/handlerUtils')
const autoExpandIfNecessary = require('../utils/autoExpandToOne')
const { getEtagElement, isSingleton } = require('../../../../common/utils/csn')

const COUNT1 = JSON.parse('[{"func":"count","args":[{"val":"1"}],"as":"counted"}]')
...
const _invokeFunction = async (tx, req, odataReq) => {
  const result = await tx.emit(req)
  ...
  if (functionReturnType && functionReturnType.kind === 'entity' && odataReq.getQueryOptions()) {
    await actionAndFunctionQueries(req, odataReq, result, tx)
  }
...
const _getCount = async (tx, readReq) => {
  ...
  let result = await tx.emit(req)

  // Transform into scalar result
  result = result[0] && result[0].counted ? result[0].counted : 0
...
const _readEntityOrProperty = async (tx, req, segments) => {
  ...
  const modifiedResult = toODataResult(res[propertyElement.getName()])
 ...
 * @private
 */
const _readCollectionNoCount = async (tx, req, odataReq) => {
  const result = await tx.emit(req)
 ...
 * @private
 */
const _readCollectionWithCount = async (service, req, odataReq) => {
  req.query.SELECT.count = true

  const result = await _readCollectionNoCount(service, req, odataReq)
  result['*@odata.count'] = result.value.$count || 0
...
const _readStream = async (tx, req, segments) => {
  ...
  const contentType = await getContentType(segments, tx, req)
  if (contentType) streamObj['*@odata.mediaContentType'] = contentType

  return streamObj
}

const _readSingleton = async (tx, req) => {
  const result = await tx.emit(req)
...
const _readAndTransform = (tx, req, odataReq) => {
  ...
  if (_isCollection(segments)) {
    if (_checkIfCountToBeIncluded(odataReq)) {
      return _readCollectionWithCount(tx, req, odataReq)
    }

    return _readCollectionNoCount(tx, req, odataReq)
  }
...
const _addETag = (odataReq, req, result) => {
  if (odataReq.getConcurrentResource() !== null) {
    ...
    if (Array.isArray(result.value)) {
      result.value.forEach(val => (val['*@odata.etag'] = val[etagElement.name]))
    } else {
      result.value['*@odata.etag'] = result.value[etagElement.name]
    }
  }
}
...
const _postProcess = (odataReq, req, result, service) => {
  ...
  autoExpandIfNecessary(req.target, result.value || result)
  if (req.target) {
    removeContainmentKeys(service, req.target, result.value || result)
  }
}
