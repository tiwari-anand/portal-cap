 ...
 * @private
 */
const deleteToCQN = odataReq => {
  const segments = odataReq.getUriInfo().getPathSegments()
  const segment = segments[segments.length - 1]

  if (SUPPORTED_KINDS[segment.getKind()]) {
    return DELETE.from(convertUrlPathToCqn(segments))
  }

  if (segment.getKind() === 'PRIMITIVE.PROPERTY') {
    return UPDATE(convertUrlPathToCqn(segments)).data({ [segment.getProperty().getName()]: null })
  }
