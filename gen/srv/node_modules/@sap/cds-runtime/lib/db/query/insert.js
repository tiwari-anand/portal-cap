...
const insert = executeInsertCQN => async (model, dbc, query, req) => {
  ...
  if (hasDeepInsert(model && model.definitions, query)) {
    ...
    const results = await processCQNs(executeInsertCQN, cqns, model, dbc, req.user.id, req.user.locale, ts)
    return getFlatArray(results)
  }

  return executeInsertCQN(model, dbc, query, req.user.id, req.user.locale, ts)
}
