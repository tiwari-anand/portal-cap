...
const _flatToStructured = (structuredType, data, prefixes = [], subData = data) => {
  ...
  for (const element in structuredType.elements) {
    ...
    if (
      subData[structuredType.name] !== undefined &&
      (structuredType.elements[element].type === 'cds.Association' ||
        structuredType.elements[element].type === 'cds.Composition')
    ) {
...
module.exports = function (result, req) {
  ...
  const structuredTypes = _getStructuredTypes(getEntityFromCQN(req, this))
