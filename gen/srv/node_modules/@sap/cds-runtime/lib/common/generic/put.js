...
const { isReadOnlyField } = require('../utils/element')
...
const _pick = element => {
  if (!NAVIGATIONS[element.type] && !element.key && !isReadOnlyField(element)) {
    /* istanbul ignore else */
    if (element.default) {
...
function _handler (req) {
  ...
  const streamProperties = Object.keys(req.target.elements).filter(k => req.target.elements[k]['@Core.MediaType'])
  if (streamProperties.some(k => req.data[k])) return
