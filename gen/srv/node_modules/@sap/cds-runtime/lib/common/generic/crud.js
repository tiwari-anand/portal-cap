  ...
  return (row, key, element, plain, isRoot) => {
    ...
    for (let category of categories) {
      if (category === '@cds.on.update' || (event === 'CREATE' && category === '@cds.on.insert')) {
        if (row[key] === '$user') row[key] = user.id
        else if (row[key] === '$now') row[key] = ts
      }
    }
  }
}
...
const _updateReqData = (req, that) => {
  ...
  if (template.elements.size > 0) {
    ...
    for (const row of arrayData) {
      templateProcessor(_processorFn(req), row, template)
    }
  }
}
...
module.exports = function () {
  this.on(['CREATE', 'READ', 'UPDATE', 'DELETE'], '*', async req => {
    ...
    if (
      ...
      req.target['@cds.persistence.skip'] &&
      req.target['@cds.persistence.skip'] !== 'if-unused'
    ) {
