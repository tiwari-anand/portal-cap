const { getOnCondElements } = require('./composition/backlinks')

const _isNavigation = element => element && (element.type === 'cds.Association' || element.type === 'cds.Composition')
...
const _buildWhereForNavigations = (ref, newWhere, model, target) => {
  ...
  if (nextRef) {
    const cnsEntity = target || model.definitions[currentRef.id || currentRef]
    const navigationElement = cnsEntity && cnsEntity.elements[nextRef.id || nextRef]
    ...
    for (const key of nextKeys) {
      ...
      if (_isNavigation(targetKeyElement)) {
