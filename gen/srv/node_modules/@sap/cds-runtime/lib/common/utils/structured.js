...
function _isAssocToOneManagedKey (refElement, element) {
  return (
    element &&
    refElement.ref &&
    refElement.ref[0] === element.name &&
    element.is2one &&
    !element.on &&
    element._target.elements[refElement.ref[1]]
  )
}
...
const _flattenStructuredWhereHaving = (filterArray, csnEntity) => {
  if (filterArray) {
    ...
    for (let i = 0; i < filterArray.length; i++) {
      ...
      } else if (_isAssocToOneManagedKey(refElement, element)) {
        newFilterArray.push({ ref: [refElement.ref.join('_')] })
      } else {
