...
const LOG = (cds.log || cds.debug)('hana')
...
function writeStreamWithHdb (dbc, sql, values) {
  LOG && LOG._debug && LOG.debug(`${sql} ${values && values.length ? '[<stream>]' : ''}`) // NOSONAR
...
function readStreamWithHanaClient (dbc, sql, values) {
  LOG && LOG._debug && LOG.debug(`${sql} ${values && values.length ? '[<stream>]' : ''}`)
  return new Promise((resolve, reject) => {
    // NOSONAR
