...
const { hasJavaNature, isOldJavaStack, getProperty } = require('./util')
...
class BuildTaskFactory {
    ...
    async getTasks(buildOptions = { root: process.cwd() }) {
        try {
            ...
            if (!buildOptions.cli) {
                cli.log(e, { withStack: true, log: (message) => this.logger.log(message) })
            }
            throw e
        }
    }
    ...
    async _createTasks(buildOptions) {
        ...
        tasks = this._ensureConsistentSettings(tasks, projectPath)
    ...
    _getExistingTasks() {
        ...
        if (Array.isArray(this.env.build.tasks)) {
            tasks = JSON.parse(JSON.stringify(this.env.build.tasks))
        }
        return tasks
    }

    _ensureConsistentSettings(tasks, projectPath) {
        let srv
        ...
        const normalizedTasks = tasks.map((task) => {
            ...
            if (BuildTaskHandlerFactory._getForValueFromTask(task) === BUILD_TASK_JAVA) {
                srv = task.src
            }
        ...
        if (!srv) {
            srv = BuildTaskFactory._getModuleFolder(projectPath, BuildTaskFactory._flatten([this.env.folders.srv])) || "srv"
        }
        
        // Java projects use "." as the default build target folder
        if (this._hasJavaNature(projectPath, srv) && this._adaptBuildTargetSettingForJava(projectPath) && DEBUG) {
        ...
        if (!task.src) {
            ...
            switch (taskFor) {
                ...
                case BUILD_TASK_MTX:
                    ...
                    break
                default:
                    throw new Error(`Invalid build task definition - value of property 'src' is missing in [${taskFor}].`)
            }
            if (DEBUG) {
