...
const { BUILD_OPTION_OUTPUT_MODE, OUTPUT_MODE_DEFAULT, OUTPUT_MODE_RESULT_ONLY, FOLDER_GEN, SEVERITES } = require('./constants')
const { compareErrors, getProperty } = require('./util')
...
class BuildTaskHandler {
    ...
    getBuildOption(name) {
        ...
        let value = getProperty(this.task.options, name)
        if (value !== undefined) {
            return value
        }
        value = getProperty(this.buildOptions.for[this.task.for], name)
        if (value !== undefined) {
            return value
        }
        return getProperty(this.buildOptions, name)
    }
    /**
    ...
    /**
     * Checks for duplicate messages and appends the new ones.
     * Note: Messages will be logged after all build tasks are finished.
     * @param {*} newMessages
     */
    pushMessages(newMessages) {
        if (!newMessages || newMessages === this._messages) {
            return
        }
        if (newMessages instanceof Error) {
            newMessages = [newMessages]
        }
        if (Array.isArray(newMessages)) {
            const merged = new Set(this._messages)
            newMessages.forEach(newMessage => {
                const found = this._messages.find(message => compareErrors(message, newMessage, true) === 0)
                if (!found) {
                    merged.add(newMessage)
                } else if (newMessage.severity && found.severity) {
                    //add message with higher prio as cds compiler might have reclassified error messages from severity WARNING to ERROR
                    if (SEVERITES.indexOf(newMessage.severity) < SEVERITES.indexOf(found.severity)) {
                        merged.add(newMessage)
                        merged.delete(found)
                    }
                }
    ...
    options() {
        // fallback - customers may need to switch-off in case of regressions
        return { messages: this.env.get('features.skip_reclassification') ? [] : [...this._messages] }
    }
