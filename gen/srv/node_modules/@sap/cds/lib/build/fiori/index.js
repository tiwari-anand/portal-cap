...
const { getProperty, isOldJavaStack } = require('../util')
...
class FioriAppModuleBuilder extends BuildTaskHandlerOData {
    ...
    async prepare() {
        ...
        try {
            ...
            for (let [appFolder, appModelGroup] of appModelGroups.entries()) {
                this.logger.log(`[cds] - building module [${appFolder}] using [${this.name}]`)
    ...
    async build() {
        ...
        if (!this.hasBuildOption(BUILD_OPTION_OUTPUT_MODE, OUTPUT_MODE_RESULT_ONLY)) {
            await this.copyNativeContent(src, dest)
        }
    }
    ...
    async _writeEdmxToWebapp(src, dest) {
        ...
        try {
            ...
            this.logger.log(`[cds] - UI module does not contain a manifest.json [${this.stripProjectPaths(manifestPath)}], skipping build`)
            return
        }
        ...
        if (!mainService) {
            if (DEBUG) {
                ...
                this.logger.log(`[cds] - UI module does not have a datasource [mainService], [${this.stripProjectPaths(manifestPath)}], skipping build`)
            }
            return
        }
        ...
        if (!localUri || !uri) {
            if (DEBUG) {
                this.logger.warn(`[cds] - local uri setting missing for data source [mainService], [${this.stripProjectPaths(manifestPath)}]`)
            }
            return
        }
