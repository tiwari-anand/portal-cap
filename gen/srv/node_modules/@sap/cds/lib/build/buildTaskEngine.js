...
const { allSettled, compareErrors, BuildError, BuildMessage } = require('./util')
...
class BuildTaskEngine {
    ...
    _sortCompilationErrorsUnique(buildOptions, ...messages) {
        ...
        messages.forEach(message => {
            const hasDup = filteredMessages.some(v => compareErrors(v, message) === 0)
            if (!hasDup && (!message.severity || logLevelIdx >= SEVERITES.indexOf(message.severity))) {
                filteredMessages.push(message)
            }
        })
        return filteredMessages.sort(compareErrors)
    }
