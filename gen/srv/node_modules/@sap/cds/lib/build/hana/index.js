...
const { OUTPUT_MODE_RESULT_ONLY, BUILD_OPTION_OUTPUT_MODE, FILE_EXT_CDS, SEVERITY_ERROR } = require('../constants')
...
const REQUIRED_PLUGIN_TYPES = [FILE_EXT_CSV, FILE_EXT_HDBTABLEDATA]
class HanaModuleBuilder extends BuildTaskHandlerOData {
    ...
    async _compileToHdbtabledata(model, dest) { //NOSONAR
        ...
        if (csvFiles.length > 0) {
            ...
            const promises = []
            const tableDatas = await this.cds.compile.to.hdbtabledata(model, { dirs: csvDirs, baseDir: this.task.options.compileDest })
    ...
    async _stripCsvComments(csvFiles) {
        ...
        for (const file of csvFiles) {
            if (this.isStagingBuild() || file.startsWith(this.task.options.compileDest)) {
                ...
                await CSV.stripComments(Readable.from([content]), fs.createWriteStream(file))
            }
        }
    }
    ...
    async _compileToHana(model) {
        ...
        if (!this.hasCdsEnvOption('features.journal', true) || format === 'hdbcds') {
            await this._compileToHdb(model, pluginTypes, format, options)
        } else {
            await this._compileToHdbmigrationtable(model, pluginTypes, options)
        }
        return pluginTypes
    }
    ...
    async _compileToHdb(model, pluginTypes, format, options) {
        ...
        this.pushMessages(options.messages)
        const promises = []
    ...
    async _compileToHdbmigrationtable(model, pluginTypes, options) {
        const relDest = path.relative(this.task.dest, this.task.options.compileDest)
        const dbSrcDir = path.join(this.task.src, "src")
        const csnPath = path.join(this.task.src, 'lastDevVersion/csn.json')
        let lastDevVersion = model
        if (await fs.pathExists(csnPath)) {
            lastDevVersion = await fs.readJSON(csnPath, 'utf-8')
        }
        const { hdiArtifacts, targetState } = await this.cds.compile.to.hdbmigrationtable(model, lastDevVersion, dbSrcDir, options)
        this.pushMessages(options.messages)
        const promises = []

        for (const { content, file, changed } of hdiArtifacts) {
            const extName = path.extname(file)
            pluginTypes.add(extName)

            if (extName === '.hdbmigrationtable') {
                if (changed) {
                    ...
                    this.logger.log(`No change reported for ${file}`)
                }
            } else {
                this._result.hana.push(path.join(relDest, file))
                if (!this.hasBuildOption(BUILD_OPTION_OUTPUT_MODE, OUTPUT_MODE_RESULT_ONLY)) {
                ...
                if (extName === '.hdbtable') {
                    ...
                    if (await fs.pathExists(path.join(dbSrcDir, entityName + '.hdbmigrationtable'))) {
                        this.pushMessages(new BuildMessage(`A .hdbmigrationtable file already exists for model entity ${entityName}. Either annotate the entity with '@cds.persistence.journal' or undeploy the file ${path.join('src', entityName + '.hdbmigrationtable')}.`, SEVERITY_ERROR))
                    }
                }
            }
        }
        ...
        if (targetState && !this.hasBuildOption(BUILD_OPTION_OUTPUT_MODE, OUTPUT_MODE_RESULT_ONLY)) {
            await this.write(targetState).to(csnPath)
        }
    }
