...
const Properies = require('../utils/properties')
...
class Config {
   ...
   * @returns {Config}
   */
  for (context, cwd=process_cwd, _force_cache_update=false, _filter_default_values=false) {
   ...
   */
  get "defined-profiles" () { return Array.from (this._profiles_defined) }

  get effective(){
    const odata = _effective_odata.bind(this)
    const cdsc = _effective_cdsc.bind(this)
    const value = new class {
      get cdsc() { return super.cdsc = Object.assign(cdsc,cdsc()) }
      get odata() { return super.odata = Object.assign(odata,odata()) }
    }
    Object.defineProperty(this,'effective',{value})
    return value
  }
...
//
  ...
  _add_to_process_env (cwd, filename) {
    ...
    try {
      const src = fs.readFileSync(require.resolve(file),'utf8'); if (!src) return
      const all = src.match(/^\s*{/) ? JSON.parse(src) : Properies.parse(src)
...
function _effective_odata(o) { // NOSONAR
  if (o && o.odata) o = { ...o, ...o.odata }
  if (o && o.flavor && !(o.flavor in this.odata.flavors)) throw cds.error(`Unknown odata.flavor: '${o.flavor}'`)
  const conf = this.odata, flavor = conf.flavors && conf.flavors [o && o.flavor || conf.flavor]
  const odata = { ...flavor, ...conf, ...o }
...
function _effective_cdsc(o) { // NOSONAR
  const odata = _effective_odata.call(this,o), cdsc = {...this.cdsc}
  let x
  if ((x = odata.version)     !== undefined && !('odataVersion'         in cdsc)) cdsc.odataVersion = x
  if ((x = odata.containment) !== undefined && !('odataContainment'     in cdsc)) cdsc.odataContainment = x
  if ((x = odata.safe_ax)     !== undefined && !('dependentAutoexposed' in cdsc)) cdsc.dependentAutoexposed = x
  if ((x = odata.structs)     !== undefined && !('odataFormat'          in cdsc)) cdsc.odataFormat = x ? 'structured' : 'flat'
