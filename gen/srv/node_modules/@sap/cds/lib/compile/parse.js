...
const parse = module.exports = Object.assign (cds_parse, {
  ...
  cdl: (src,o) => cds.compile.to.parsed.csn(src,o),
  ...
  path: x => {
    const [ , head, filter, tail ] = /^([^[:]+)(?:\[(.+)\])?(?::(.*))?/.exec(x)
    let ref = filter ? [{ id:head, where:cds.parse.expr(filter) }] : [head]
    if (tail) ref.push (...cds.parse.ref(tail))
    return {ref}
  },
