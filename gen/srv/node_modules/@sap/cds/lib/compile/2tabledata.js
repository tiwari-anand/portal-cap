...
const {getArtifactCdsPersistenceName, getElementCdsPersistenceName} = require ('./_cdsc')
const readdir = cds.utils.promisify (cds.utils.readdir)
...
async function _tabledata4 (dir, csvFile, model, baseDir)  {
  ...
  const _import = {
    target_table: getArtifactCdsPersistenceName (entity.name, cds.env.sql.names),
  ...
  cols.forEach(csvCol => {
    ...
    const tableCol = getElementCdsPersistenceName (csvCol, cds.env.sql.names)
  ...
  if (entity.elements.locale)  {
    ...
    if (locale) {
      const localeKey = getElementCdsPersistenceName (entity.elements.locale.name/*usually 'LOCALE'*/, cds.env.sql.names)
