...
const factory = require('./srv/factory')
const _pending = {} // used below to chain parallel connect.to(<same>)
 ...
 * @returns { Promise<import('./srv/Service-api')> }
 */
connect.to = async (datasource, options) => { //NOSONAR
	let Service = factory, _done = noop
	if (typeof datasource === 'object') [options,datasource] = [datasource]
		...
		_pending[datasource] = new Promise (resolve => _done=resolve).then(srv => {
	...
	delete _pending[datasource]; _done(srv)
