const { ProtocolAdapter } = require('./srv/adapters')
const { Service } = require('./srv/factory')
const cds = require ('.')

// Prepare dictionary of external services
const external = {__proto__: cds.requires, _init(){
  for (let e of Object.values(cds.requires)) if (e.service)  this[e.service] = e
  return this
}}._init()
...
function cds_serve (som, _options) { // NOSONAR
  ...
  const all=[], provided = loaded.then (csn => { // NOSONAR
    ...
    if (o.service && o.service !== 'all' && !services.find(s=>s.name.endsWith (o.service))) {
      throw cds.error (`No such service '${o.service}' among [${services.map(s=>s.name)}]`)
    }
    const defs = services.filter (d => !(
      // skip bound external services
      external[d.name] && external[d.name].credentials ||
      // skip external services unless asked to mock them
      external[d.name] && !o.mocked ||
      // skip the ones not chosen by o.service
      o.service && o.service !== 'all' && !d.name.endsWith (o.service) ||
      // skip all marked to be ignored
      d['@cds.ignore'] || d['@cds.serve.ignore']
    ))
    if (( o.at || o.with ) && defs.length > 1) {
    ...
    all.push (...defs.map(d => _new (Service,d,csn,o)))
  })
  ...
  let ready = provided.then (()=> Promise.all (all.map (async srv => {
    await srv.prepend (srv.init)
    await srv.prepend (srv.options.impl)
    return srv
  })))
