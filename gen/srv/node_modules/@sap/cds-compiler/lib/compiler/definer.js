...
const {
  makeMessageFunction, searchName, weakLocation, hasErrors,
} = require('../base/messages');
const {
  isBetaEnabled,
  queryOps, setProp, forEachGeneric, forEachInOrder,
...
function getDefinerFunctions( model ) {
  ...
  function initSource( src ) {
    ...
    if (!namespace) {
      ...
      if (builtin && !builtin.internal && absolute !== 'cds') {
        warning( 'ref-shadowed-builtin', [ namespace.location, null ], // no home artifact
  ...
  /**
   * Set property `name.absolute` or artifact `art` to `absolute`.  Also
   ...
   * @param {string} [absolute=art.name.absolute]
   * @param {string} [prefix]
   * @param {XSN.Artifact} [parent]
   * @param {boolean|number} [noReservedCheck]
   */
  ...
  function initUsings( src, usings ) {
    ...
    for (const def of usings) {
      ...
      if (ue.path[0] && ue.path[0].id === 'localized') {
        ...
        warning( 'using-localized-view', [ ue.location, ue ], {},
        ...
        addToDict( src.artifacts, def.name.id, def, ( name, loc, art ) => {
          if (art.kind === 'using') { // repeated defs would be shown repeatedly otherwise
            error( 'duplicate-using', [ loc, null ], { name },
  ...
  function initArtifacts( construct, parent, block, collectExts,
    ...
    //   return;
    // TODO: the checkDefinitions must be probably checked by the parsers
    // let dottedNames = [];
    ...
    function init( art ) {
      ...
      if (kindProperties[art.kind].artifacts) {
        ...
        if (art.dbType && !options.hanaFlavor)
          message( null, [ art.dbType.location, art ], 'TABLE TYPE is not supported yet', 'Error' );
        if (art.kind === 'annotation') {
          message( 'anno-definition', [ art.name.location, art ], {},
                   'CSN representation of annotation definitions will change', 'Warning' );
          if (art._parent && [ 'context', 'service' ].includes(art._parent.kind)) {
            message( 'anno-top-level', [ art.name.location, art ], {},
                     'Annotations must be defined top-level and not inside a context/service',
                     'Warning');
          }
        }
  ...
  function initQueryExpression( art, query, parents = [] ) {
    ...
    function signalDuplicate( name, loc ) {
      error( 'duplicate-definition', [ loc, query ], { name, '#': '$tableAlias' },
    ...
    function addAlias( alias, subquery ) {
      ...
      if (!query.name || !query.name.id) {
        error( 'query-req-alias', [ query.location, query ], {}, // TODO: not subquery.location ?
    ...
    function init( elem, name, prop ) {
      ...
      if (parent.enum && elem.type) {
        ...
        error( 'unexpected-type', [ elem.type.location, elem ], {},
               'Enum values cannot have a custom type' );
      }
    }
  }
   ...
   * TODO v2: remove _parent link for artifacts (_block is relevant for name resolution)
   *
  ...
  function setAncestorsAndService( name ) {
    ...
    const s = parent && (parent._service || parent.kind === 'service' && parent || null);
    ...
    if (art.kind === 'service') {
      ...
      message( 'service-nested-service', [ art.name.location, art ], { art: parent },
               'A service cannot be nested within a service $(ART)', [ 'Error' ] );
    }
    else if (art.kind === 'context') {
      while (parent.kind !== 'service')
        parent = parent._parent;
      message( 'service-nested-context', [ art.name.location, art ], { art: parent },
               'A context cannot be nested within a service $(ART)', [ 'Error' ] );
    }
  }
  ...
  function postProcessArtifact( art ) {
    ...
    if (!art._ancestors || art.kind !== 'entity' && art.kind !== 'view')
      return;
  ...
  function processAspectComposition( base ) {
    ...
    function expand( elem ) {
      ...
      if (!target.elements) {
        ...
        error( null, [ target.location, elem ], {},
  ...
  function collectArtifactExtensions( construct, block ) {
    for (const ext of construct.extensions || []) {
      ...
      if (ext.elements)
        Object.keys(ext.elements).forEach( elem => setProp( ext.elements[elem], '_parent', ext ) );
  ...
  function initExtensionsWithoutApplying() {
    ...
    function recursivelyResolveExpressionCastTypes( expr, artifact ) {
      for (const arg of expr.args || []) {
        if (arg.type) {
          const name = resolveUncheckedPath(arg.type, 'type', artifact);
          const def = name && model.definitions[name];
          if (def)
            resolveTypeArguments( arg, def, artifact );
        }
    ...
    function resolveTypeUnchecked(artWithType, artifact) {
      ...
      if (artWithType.type.scope !== 'typeOf') {
        ...
        message( 'ref-undefined-typeof', [ artWithType.type.location, artifact ], {},
   ...
   * fake "built-in" location will be used.
   *
   * @param {string} name
  ...
  function serviceIncludes( art ) {
    ...
    if (art.includes) {         // service includes - just resolve them now
      message( null, [ dictLocation( art.includes ), art ],
               'Service includes are not supported yet', 'Error' );
      ...
      if (!('_artifact' in ext.name)) { // not already applied
        ...
        if (noExtend && ext.kind === 'extend') {
          error( 'extend-for-generated', [ ext.name.location, ext ], { art },
                 'You cannot use EXTEND on the generated $(ART)' );
          continue;
        }
        if (ext.includes) {
      ...
      if (!open.length) {
        ...
        if (lastExt) {
          message( 'extend-repeated-intralayer', [ lastExt.location, lastExt ], {},
                   'Unstable element order due to repeated extensions in same layer', 'Warning' );
          lastExt = null;
        }
        ...
        if (lastExt && (open.length > 1 || !extLayer._layerExtends[open[0]])) {
          ...
          message( 'extend-unrelated-layer', [ lastExt.location, lastExt ], {},
                   'Unstable element order due to other extension in unrelated layer', 'Warning' );
        }
        lastExt = ext;
  ...
  function extendNothing( extensions, prop, name, art, validDict ) {
    for (const ext of extensions) {
      ...
      if (options.testMode) {
        ...
        info(null, [ location ], names.length ? `Valid: ${ names.sort().join(', ') }` : 'No valid names');
      }
    }
  }
  ...
  function preProcessArtifact( name ) {
    ...
    if (art instanceof Array) {
      // At this stage we have redefinitions but it is possible that (at least) one
      // definition has an invalid name, i.e. contains a '.'.  To avoid a confusing
      // error message we check here that all identifiers are also valid.
      art.forEach((a) => {
        if (a.name && a.name.id && a.name.id.includes('.')) {
          error( 'syntax-invalid-ident', [ a.name.location, a ], {},
                 'Identifier must not contain path separator \'.\'' );
        }
        else {
          checkRedefinitions( a, name, 'definitions', true );
        }
      });
    }
    else {
  ...
  function localizedData( art, textsEntity, fioriEnabled ) {
    ...
    if (textsEntity) {
      ...
      if (textsEntity.kind !== 'entity' || textsEntity.query ||
        ...
        info( null, [ art.name.location, null ], {},
  ...
  function createTextsEntity( base, absolute, textElems, fioriEnabled ) {
    ...
    if (addTextsLanguageAssoc) {
      ...
      addToDictWithIndexNo( art, 'elements', 'language', language );
    }
    const artifacts = Object.create(null);
