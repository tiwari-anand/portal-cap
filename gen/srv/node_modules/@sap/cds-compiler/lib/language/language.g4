...
tokens {
  VIRTUAL,                      // used with setLocalKeyword()
...
@after { this.attachLocation($art); }
  :
    ...
    ( TYPE | ASPECT ) simplePath[ $name, 'Extend' ]
    { $art = this.addItem( $outer, 'extensions', 'extend', $annos,
...
@after{ /* #ATN 1 */ this.attachLocation($art); }
  :
    ...
    // VIRTUAL is keyword, except if before the following tokens texts:
    { this.setLocalKeyword( 'VIRTUAL', [ ',', '.', ':', '[', '@', 'AS' ] ); } // not '{'
    virtual=VIRTUAL?
    key=KEY?
    (
      e=expression
      { $art = this.addItem( $outer, 'columns', null, $annos, { value: $e.expr, key: $key, virtual: $virtual }); }
...
tableExpression returns[ query ] // TableOrJoin
  :
    ...
    (
      ...
      te=tableExpression
      {
        $query = { op: $join.op, join: $join.join, args: [$query, $te.query] };
        if($join.joinCard)
          $query.cardinality = $join.joinCard;
      }
      ON cond=condition
      {
        $query.on = $cond.cond;
...
joinOp returns [ op, join, joinCard ]
  :
    ( j=JOIN                  // double ;; (and after ')') is ANTLR bug workaround
      { $op = this.tokenLocation( $j, undefined, 'join' );; $join = 'inner'; }
    | i=INNER c=joinCardinality? j=JOIN
      { $op = this.tokenLocation( $i, $j, 'join' );; $join = 'inner';; $joinCard = $ctx.c && $c.joinCard; }
    | l=LEFT OUTER? c=joinCardinality? j=JOIN
      { $op = this.tokenLocation( $l, $j, 'join' );; $join = 'leftOuter';; $joinCard = $ctx.c && $c.joinCard; } // left
    | r=RIGHT OUTER? c=joinCardinality? j=JOIN
      { $op = this.tokenLocation( $r, $j, 'join' );; $join = 'rightOuter';; $joinCard = $ctx.c && $c.joinCard; } // right
    | f=FULL OUTER? c=joinCardinality? j=JOIN
      { $op = this.tokenLocation( $f, $j, 'join' );; $join = 'fullOuter';; $joinCard = $ctx.c && $c.joinCard; }  // full
    )
  ;
...
@after{ /* #ATN 1 */ }
  ...
  |
    ...
    {
      $ret = {
        op: this.tokenLocation( $ca, undefined, 'xpr' ),
        args: [ ],
        location: this.tokenLocation( $ca )
      };
    }
    e=expression AS typeRefOptArgs[ $e.expr ]
    { 
      $e.expr._typeIsExplicit = true;
...
namedValue[ struct ] locals[ namedVal = { name: {} } ]
  :
    // at='@'? // not possible yet as it would not allowed reserved keywords
    annotationPath[ $namedVal.name, 'name' ]
