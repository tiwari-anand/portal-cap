...
function translateAssocsToJoinsCSN(csn, options){
  ...
  const model = compileSources( { [file]: xsn }, opt );
  timetrace.stop();
  ...
  // Make sure that we keep pre-existing messages and keep the ref to the original csn.messages intact
  if(model.messages && !options.messages){
    // Make sure that we don't complain twice about the same things
    model.messages.forEach(m => csn.messages.push(m));
    deduplicateMessages( csn.messages );
  } else {
    // Make sure that we don't complain twice about the same things
...
function translateAssocsToJoins(model, inputOptions = {})
{
          ...
          (pathDict, env) => {
            ...
            env.full =
              ...
              || pathDict.path.some((e, i, a) => {
                ...
                const hasArgs = !!e.namedArgs;
                const hasCardinality = e.cardinality;
                const isAssocStep = e._artifact.target;
                const isUDF = isAssocStep && isBooleanAnnotation(e._artifact.target._artifact['@cds.persistence.udf'], true) || isBooleanAnnotation(e._artifact['@cds.persistence.udf'], true);
                const isCV = isAssocStep && isBooleanAnnotation(e._artifact.target._artifact['@cds.persistence.calcview'], true) || isBooleanAnnotation(e._artifact['@cds.persistence.calcview'], true);
                const isNotTail = a.length-1 > i;
                const isMixinUsage = e._navigation && e._navigation.$QA && e._navigation.$QA.mixin;
                return ( isUDF || isCV || (hasArgs && (isAssocStep || isNotTail)) || hasCardinality || (isMixinUsage && isNotTail));
              });
          }
      }
    ...
    else
    {
      ...
      if(QA) {
        ...
        if(QA.mixin && path.length == 1)
          return;
          // tail is to be appended to the QA (table alias)
        const tail = path.slice(path.indexOf(ps));
        // check from left to right if a subsequent QAT is $njr
        // if so, substitute path with pregenerated foreign key

        // can be activated if forHanaNew rewrites paths correctly
        // if(!(tail[tail.length-1]._artifact._finalType.elements ||
        //      tail[tail.length-1]._artifact._finalType.target)) {
        for(let i = 0; i < tail.length-1; i++) {
          if(tail[i]._navigation.$njr) {
            // the correct flattened foreign key must match the leaf artifact of this path

            // check origin._artifact on both ends and compare the origin._artifact with the counterpart
            // all four combinations. Due to complications with localized elements it could be that any
            // combination of _artifact and/or origin._artifact is a match.
            const leafArt = tail[tail.length-1]._artifact;
            const fk = tail[i]._artifact.$flatSrcFKs.find(f => {
              const flatArt = f._artifact;
              return (flatArt === leafArt ||
                   flatArt.origin && flatArt.origin._artifact === leafArt ||
                   leafArt.origin && leafArt.origin._artifact === flatArt ||
                   flatArt.origin && flatArt.origin._artifact === leafArt.origin && leafArt.origin._artifact)
            });
            if(!fk) {
            ...
            tail.splice(i, tail.length, fk); // will terminate for loop
          }
        }
        // }

        pathValue = constructPathNode([ constructTableAliasPathStep(QA), ...tail ]);
      }
      else {
  ...
  */
  function createJoinTree(env, joinTree, parentQat, joinType, qatAttribName, lastAssocQA)
  {
    for(let childQatId in parentQat)
    {
      ...
      if(!childQat.$njr) {
        ...
        const art = childQat.origin._artifact;
  ...
  function createJoinQA(joinType, lhs, rhs, assocQAT, assocSourceQA, env)
  {
    ...
    const assoc = assocQAT.origin._artifact;
    if(isBetaEnabled(options, 'mapAssocToJoinCardinality')) {
      node.cardinality = mapAssocToJoinCardinality(assoc.cardinality);
    }
    ...
    /*
        ...
        Default is the CDS default:
        sourceMax = *, targetMax = 1 > MANY TO ONE
    */
    function mapAssocToJoinCardinality(csnCard) {
      /** @type {object} */
      const xsnCard = {
        sourceMax : { literal: 'string', val: '*' },
        targetMax : { literal: 'number', val: 1 }
      };
      if(csnCard) {
        if(csnCard.sourceMax && csnCard.sourceMax.val === 1) {
          xsnCard.sourceMax.literal = 'number';
          xsnCard.sourceMax.val = 1;
        }
        if(csnCard.targetMax && csnCard.targetMax.val !== 1) {
          xsnCard.targetMax.literal = 'string';
          xsnCard.targetMax.val = '*';
        }
        else if(csnCard.targetMin && csnCard.targetMin.val === 1)
    ...
    function createOnCondition(assoc, srcAlias, tgtAlias)
    {
      ...
      */
      ...
      if(assoc.foreignKeys)
      {
        ...
        */
        if(!assoc.$flatSrcFKs)
          setProp(assoc, '$flatSrcFKs', flattenElement(assoc, true, assoc.name.element.replace(/\./g, pathDelimiter)));
        if(!assoc.$flatTgtFKs)
        ...
        */
        ...
        {
          args.push({op: {val: '=' },
            args: [ constructPathNode( [ srcAlias, assoc.$flatSrcFKs[i] ] ),
        ...
        if(env.assocStack === undefined) {
          ...
          env.assocStack.stripAssocPrefix = function(path) {
            ...
            if(elt) {
              ...
              if(epath.length < path.length) {
                for(let i = 0; i < epath.length && found; i++) {
                  found = epath[i] === path[i].id;
                }
                if(found)
                  return path.slice(epath.length);
              }
            }
            if(id) {
              let found = true;
              ...
              */
              ...
              path.forEach(ps => { // eslint-disable-line consistent-return
                if(ps._artifact.target) {
                  signal(error`"${env.lead.name.absolute}": "${ps.id}" in path "${pathAsStr(pathNode.path)}" must not be an association`, pathNode.location);
  ...
  */
  function checkPathDictionary(pathDict, env)
  {
    ...
    const artifact = lastSegment._artifact.type && lastSegment._artifact.type._artifact && lastSegment._artifact.type._artifact;
    if (artifact && artifact.elements) {
      signal(error`"${lead.name.absolute}": "${lastSegment.id}" must have scalar type`, pathDict.location);
      pathDict.$check = false;
    }
    return pathDict.$check;
  }
  ...
  */
  function mergePathIntoQAT(pathDict, env)
  {
    ...
    let isJoinRelevant = false;
    let hasNonJoinRelevantQATs = true;
    tail.forEach((pathStep, i) => {
      ...
      */
      ...
      /*
        ...
        optionally
        6) Association is exactly to one [1..1]
      ...
      */
      const art = qat.origin._artifact;
      if(noJoinForFK && qat.origin._artifact.target) {
        if(!pathStep.where && // has no filter
          !pathStep.namedArgs && // has no args
          i < tail.length-1 && // not leaf
          // path terminates on a scalar type
          // _finalType.elements can be removed if forHanaNew can expand fk paths correctly
          !(tail[tail.length-1]._artifact._finalType.elements || tail[tail.length-1]._artifact._finalType.target ) &&
          // association is managed
          art.foreignKeys &&
          // n+1st path step is foreign key
          art.foreignKeys[tail[i+1].id])
        {
          ...
          if(!qat[qatChildrenName]) {
            ...
            setProp(art, '$flatSrcFKs', flattenElement(art, true, art.name.element.replace(/\./g, pathDelimiter)));
          }
        }
        else {
          // join relevant latch, set only for assoc path steps
          isJoinRelevant = true;
        }
      }
      // non join relevant latch to optimize njr->jr unmarking
      hasNonJoinRelevantQATs = hasNonJoinRelevantQATs || !!qat.$njr;
    ...
    */
    ...
    // unmark QAT root path that contains njr QATs
    if(isJoinRelevant && hasNonJoinRelevantQATs) {
      do {
        delete qat['$njr'];
        qat = qat._parent;
