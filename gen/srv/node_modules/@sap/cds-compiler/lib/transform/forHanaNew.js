...
const { rejectParamDefaultsInHanaCds } = require('../checks/csn/defaultValues');
...
const checkUsedTypesForAnonymousAspectComposition = require('../checks/csn/managedInType');
...
function transformForHanaWithCsn(inputModel, options) {
  ...
  let error, warning, info, // message functions
    ...
    addDefaultTypeFacets, resolvePath, flattenPath, expandStructsInOnConditions, toFinalBaseType; // transformUtils

  bindCsnReference();

  const dialect = options.forHana && options.forHana.dialect || options.toSql && options.toSql.dialect;
  const doA2J = !options.forHana.keepStructsAssocs && options.forHana.associations !== 'assocs';
  ...
  const cleanup = validator(csn, {
    error, warning, info, inspectRef, effectiveType, artifactRef, csn,
  },
    /* Member Validators   */ [ validateOnCondition,validateForeignKeys, validateAssociationsInArrayOf, rejectParamDefaultsInHanaCds ],
    /* artifact validators */ [ validateCdsPersistenceAnnotation, function(artifact) {
      forEachMember(artifact, checkUsedTypesForAnonymousAspectComposition.bind(this));
    }],
 ...
 // check unique constraints - further processing is done in rewriteUniqueConstraints`
  ...
  const {
    ...
    recurseElements, copyTypeProperties
  ...
  forEachDefinition(csn, (art, artName) => handleManagedAssociationFKs(art, artName, createCallbacks));
  createCallbacks.forEach(fn => fn());
  ...
  const killers = {
    '_ignore': function (parent, a, b, path){
      // cds.persistence.exists needs to survive for the renderer
      if((parent.kind === 'view' || parent.kind == 'entity') && hasBoolAnnotation(parent, '@cds.persistence.exists')) {
  ...
  function bindCsnReference(){
    ...
    ({ error, warning, info } = alerts.makeMessageFunction(csn, options));
  }
  ...
  function recursiveChecks(artifact, artifactName) {
    if (!artifact._ignore) {
      forEachMemberRecursively(artifact, (member, memberName, property, path) => {
        if( options.toHana && isStructured(member) && property === 'params' ){
          error(null, path, 'View parameter type must be scalar');
          return;
        }
        else if (options.forHana.keepStructsAssocs &&
          artifact.query &&
          isStructured(member)) {
          error(null, path, `With "hdbcds" naming, structured elements cannot be used in a view`);
          return;
        }
  ...
  function handleDBChecks(artifact, artifactName) {
    ...
    forEachMemberRecursively(artifact, (member, memberName, prop, path) => {
      ...
      if (member.key && member.notNull === false)
        error(null, path, `"${ artifactName }.${ memberName }": Key elements cannot be nullable in HANA CDS`);
      ...
      if (options.forHana.dialect === 'hana' && member.target && isAssocOrComposition(member.type) && !isBetaEnabled(options, 'assocsWithParams')) {
        if (artifact.params) {
          ...
          error(null, path, `"${ artifactName }.${ memberName }": Associations are not allowed in entities with parameters`);
        }
        else if(artifact['@cds.persistence.udf'] || artifact['@cds.persistence.calcview']) {
          // UDF/CVs w/o params don't support 'WITH ASSOCIATIONS'
          error(null, path, `"${ artifactName }.${ memberName }": Associations are not allowed in entities annotated with @cds.persistence { udf, calcview }`);
        }
        if (csn.definitions[member.target].params) {
          ...
          error(null, path, `"${ artifactName }.${ memberName }": Associations cannot point to entities with parameters`);
        }
          ...
          error(null, path, `"${ artifactName }.${ memberName }": Associations cannot point to entities annotated with @cds.persistence { udf, calcview }`);
        }
      }
    }, [ 'definitions', artifactName ]);
  }
  ...
  function handleAssociations(artifact, artifactName) {
    ...
    if (artifact.kind === 'entity' || artifact.kind === 'view') {
      forAllElements(artifact, artifactName, (parent, elements, pathToElements) => {
        for (const elemName in elements) {
          ...
          if (!options.forHana.keepStructsAssocs) {
            if (isManagedAssociationElement(elem))
              transformManagedAssociation(parent, artifactName, elem, elemName, pathToElements);
          }
        }
      })
    }
  ...
  function handleChecksForWithParameters(artifact, artifactName) {
    if (!artifact._ignore && artifact.params && (artifact.kind === 'entity' || artifact.kind === 'view')) {
      if (!artifact.query) { // table entity with params
        error(null, [ 'definitions', artifactName ], `"${ artifactName }": Table-like entities with parameters are not supported for conversion to ${ options.toSql ? 'SQL' : 'HANA CDS' }`);
      }
      else if (options.forHana.dialect !== 'hana') { // view with params
        error(null, [ 'definitions', artifactName ], `"${ artifactName }": Entities with parameters are only supported on HANA`);
      }
      else {
  ...
  function handleAssocToJoins() {
    ...
    forEachDefinition(csn, (art, artName) => {
      ...
      if (art.query && newArt && newArt.query) {
  ...
  */
  ...
  function transformEntityOrViewPass2(query, artifact, artName, type, path) {
            ...
            if (colLocation) {
              ...
              if (element.target && isMixin)
                error(null, path.concat([ 'columns', colLocation ]),
                  `Element "${ elementName }" is a mixin association${ possibleMixinName !== elementName ? ` ("${ possibleMixinName }")` : '' } and cannot be published in a UNION`);
            }
          }
        }
      }
    ...
    for (const elemName in elements) {
      ...
      if (options.forHana.keepStructsAssocs &&
        ...
        !elem.on) {
        error(null, [ 'definitions', artName, 'elements', elemName ], `With "hdbcds" naming, managed association elements cannot be used in a view`);
        continue;
      }
      ...
      if (query !== undefined && elem.target) {
        if(isUnion(path) && options.toHana){
          if(isBetaEnabled(options, "ignoreAssocPublishingInUnion") && doA2J){
            ...
            error(null, path, `Association "${elemName}" cannot be published in a HANA CDS UNION.`)
          }
        } else {
          /* Old implementation:
    ...
    if (query && !hasNonAssocElements) {
      ...
      error(null, [ 'definitions', artName ], `"${ artName }": For HANA CDS or SQL, a view or projection must have at least one element that is not an unmanaged association`);
    }
  ...
  function replaceEnumSymbolsByValues(elem, path) {
    ...
    if (elem.default && elem.default['#']) {
      ...
      if (!Enum) {
        // Not an enum at all
        error(null, path, `Enum literal "#${ elem.default['#'] }" can only be used with an enum type in HANA CDS`);
      }
      else {
  ...
  function processBacklinkAssoc(elem, elemName, art, artName, pathToOn) {
    ...
    while (i < xprArgs.length) {
      ...
      if (i < xprArgs.length - 2 && xprArgs[i + 1] === '=') {
        ...
        if (isDollarSelfOrProjectionOperand(xprArgs[i]) && isAssociationOperand(xprArgs[i + 2], pathToOn.concat([ i + 2 ]))) {
        ...
        else if (isDollarSelfOrProjectionOperand(xprArgs[i + 2]) && isAssociationOperand(xprArgs[i], pathToOn.concat([ i ]))) {
    ...
    function transformDollarSelfComparison(assocOp, assoc, assocName, elem, elemName, art, artifactName, path) {
      ...
      if (assoc.on) {
        ...
        if (containsDollarSelf)
          error(null, path, `An association that uses "$self" in its ON-condition cannot be compared to "$self"`);
      }
          ...
          if (query.groupBy[i].ref) {
            ...
            if (art && art.target) {
              if (art.$managed) {
                ...
                if (options.forHana.keepStructsAssocs) {
                  error(null, groupByPath, `With "hdbcds" naming, managed association elements cannot be used in the GROUP BY clause of a view`);
                  continue;
                }
                ...
                error(null, groupByPath, `Unmanaged association elements cannot be used in the GROUP BY clause of a view`);
                continue;
              }
            }
            else {
          ...
          if (query.orderBy[i].ref) {
            ...
            if (art && art.target) {
              if (art.$managed) {
                ...
                if (options.forHana.keepStructsAssocs) {
                  error(null, orderByPath, `With "hdbcds" naming, managed association elements cannot be used in the ORDER BY clause of a view`);
                  continue;
                }
                ...
                // This is an unmanaged association - these cannot be used in an order/group by
                error(null, orderByPath, `Unmanaged association elements cannot be used in the ORDER BY clause of a view`);
                continue;
              }
            }
            else {
    ...
    for (const elemName in artifact.elements) {
      ...
      if (elem.target && isComposition(elem.type)) {
        ...
        if (!isPartOfService(draftNodeName)) {
          warning(null, [ 'definitions', artifactName, 'elements', elemName ], `Target "${ draftNodeName }" of composition "${ artifactName }.${ elemName }" cannot be a draft node because it is not part of a service`);
          continue;
        }
  ...
  function generateDraftForHana(artifact, artifactName, draftRootName) {
    ...
    if (draftAdminDataProjection.kind !== 'entity' || !draftAdminDataProjection.elements['DraftUUID']) {
      ...
      error(null, path, `Generated entity "${ draftAdminDataProjectionName }" conflicts with existing artifact`);
    }
    ...
    if (csn.definitions[draftsArtifactName]) {
      ...
      if (!definingDraftRoot)
        error(null, [ 'definitions', artifactName ], `"${ draftsArtifactName }": Generated entity name conflicts with existing entity`);

      else
        error(null, [ 'definitions', draftRootName ], `"${ draftsArtifactName }": Entity already generated by draft root "${ definingDraftRoot }"`);
    ...
    for (const elemName in artifact.elements) {
      ...
      const copyVirtual = isBetaEnabled(options, 'dontRenderVirtualElements') ? !!options.forHana.renderVirtualElements : true;
      let elem = undefined;
      if(copyVirtual && origElem.virtual)
  ...
  function checkForArrayOf(artifact, artifactName, element, elementName) {
    ...
    function show_error(eltName) {
      error(null, [ 'definitions', artifactName, 'elements', eltName ], `"${ artifactName }.${ eltName }": Array types are not allowed as table types`);
    }
  ...
  */
        ...
        if (isStructured(elem)) {
          ...
          for (const flatElemName in flatElems) {
            ...
            if (art.elements[flatElemName])
              error(null, path, `"${ artName }.${ elemName }": Flattened struct element name conflicts with existing element: "${ flatElemName }"`);
            ...
            art.elements[flatElemName] = flatElement;
   ...
   * @param {CSN.Artifact} art
   * @param {string} artName
   * @param {Function[]} collector Array to collect the flatten-and-create functions
   */
  function handleManagedAssociationFKs(art, artName, collector) {
    if ((art.kind === 'entity' || art.kind === 'view') && !options.forHana.keepStructsAssocs) {
        ...
        forEachGeneric(parent, 'elements', (element, elemName) => {
          if (isManagedAssociationElement(element)) {
            ...
            // replace foreign keys that are managed associations by their respective foreign keys
              ...
              // Save the callback for later execution - seperate expansion vs flattening and element creation
              collector.push(() => {
                for(let i = 0; i < element.keys.length; i++){
                  const foreignKey = element.keys[i];
                  flattenRefAndCreateForeignKeyElement(parent, element, elemName, foreignKey, [ ...pathToElements, elemName, 'keys', i]);
    ...
    /**
     * Flatten the reference of the FK and create it in parent
     *
     * @param {object} parent Where to create the FK element
     * @param {object} element The managed association to create the FK for
     * @param {String} elemName The name of the managed association
     * @param {object} foreignKey The entry of assoc.keys to add
     * @param {array} path Path to the foreign key
     */
    function flattenRefAndCreateForeignKeyElement(parent, element, elemName, foreignKey, path) {
      foreignKey.ref = flattenStructStepsInRef(foreignKey.ref, path);
      // Assemble right hand side of 'assoc.key = fkey' i.e. create the elements for the foreign keys in the target artifact
      createForeignKeyElement(element, elemName, foreignKey, parent, artName, path);
    }
  }
    ...
    for (let i = 0; i < assoc.keys.length; i++) {
      ...
      if (isStructured(art)) {
        ...
        Object.keys(flat).forEach((flatElemName) => {
          ...
          const clone = cloneCsn(assoc.keys[i]);
          if (clone.as) {
        ...
        // Add the newly generated foreign keys to the end - they will be picked up later on
        // Recursive solutions run into call stack issues
        art.keys.forEach(key => assoc.keys.push(cloneAndExtendRef(key, assoc.keys[i], ref)));
      }
            ...
            index.forEach((val, idx) => {
              if (typeof val === 'object' && val.ref) {
                ...
                if (!art) {
                  ...
                  if (isFulltextIndex)
                    error(null, path, `"${ artName }": A fulltext index cannot be defined on a structured element`);
