...
function getBoundary(contentType) {
    var boundary = (contentType === null || contentType === void 0 ? void 0 : contentType.match(/.*boundary=.+/)) ? util_1.last(contentType.split('boundary='))
        : undefined;
    if (!boundary) {
...
function splitBatchResponse(response) {
    ...
    try {
        var boundary = getBoundary(util_1.pickValueIgnoreCase(response.headers, 'content-type'));
...
function splitChangeSetResponse(changeSetResponse) {
    ...
    try {
        var boundary = getBoundary(util_1.pickValueIgnoreCase(headers, 'content-type'));
...
function splitResponse(response, boundary) {
    var parts = response.split("--" + boundary).map(function (part) { return part.trim(); });
    if (parts.length >= 3) {
...
function parseBatchResponse(batchResponse) {
    return splitBatchResponse(batchResponse).map(function (response) {
        var contentType = util_1.pickValueIgnoreCase(parseHeaders(response), 'content-type');
