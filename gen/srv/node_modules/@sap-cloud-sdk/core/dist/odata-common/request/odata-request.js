...
var ODataRequest = /** @class */ (function () {
    ...
    Object.defineProperty(ODataRequest.prototype, "destination", {
        ...
        set: function (dest) {
            this._destination = dest && connectivity_1.sanitizeDestination(dest);
        },
        enumerable: false,
        configurable: true
    });
     ...
     */
    ODataRequest.prototype.relativeUrl = function (includeServicePath) {
        if (includeServicePath === void 0) { includeServicePath = true; }
        return "" + remove_slashes_1.removeTrailingSlashes(this.relativeResourceUrl(includeServicePath)) + this.query();
    };
    /**
    ...
    ODataRequest.prototype.relativeServiceUrl = function () {
        var servicePath = typeof this.config.customServicePath === 'undefined'
            ? this.config.defaultServicePath
            : this.config.customServicePath;
        return "" + remove_slashes_1.removeSlashes(servicePath);
    };
    /**
    ...
    ODataRequest.prototype.relativeResourceUrl = function (includeServicePath) {
        ...
        var baseUrl = includeServicePath
            ? remove_slashes_1.removeTrailingSlashes(this.relativeServiceUrl())
            : '';
        var url = baseUrl + "/" + this.config.resourcePath();
        return remove_slashes_1.removeLeadingSlashes(url);
    };
    /**
    ...
    ODataRequest.prototype.query = function () {
        var queryParameters = __assign(__assign({}, this.config.queryParameters()), this.config.customQueryParameters);
    ...
    ODataRequest.prototype.headers = function () {
        return __awaiter(this, void 0, void 0, function () {
            var destinationRelatedHeaders, csrfHeaders, _a, error_1;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, connectivity_1.buildHeadersForDestination(this.destination, this.config.customHeaders)];
                    case 1:
                        destinationRelatedHeaders = _b.sent();
                        if (!(this.config.method === 'get')) return [3 /*break*/, 2];
                        _a = {};
                        return [3 /*break*/, 4];
                    case 2: return [4 /*yield*/, this.getCsrfHeaders(destinationRelatedHeaders)];
                    case 3:
                        _a = _b.sent();
                        _b.label = 4;
                    case 4:
                        csrfHeaders = _a;
                        return [2 /*return*/, __assign(__assign(__assign(__assign(__assign({}, destinationRelatedHeaders), csrfHeaders), this.defaultHeaders()), this.eTagHeaders()), this.customHeaders())];
                    case 5:
                        error_1 = _b.sent();
    ...
    ODataRequest.prototype.defaultHeaders = function () {
        var customDefaultHeaders = util_1.pickIgnoreCase.apply(void 0, __spreadArrays([this.customHeaders()], Object.keys(this.config.defaultHeaders)));
        return util_1.mergeIgnoreCase(util_1.pickNonNullish(this.config.defaultHeaders), customDefaultHeaders);
    };
    /**
    ...
    ODataRequest.prototype.eTagHeaders = function () {
        if (util_1.pickValueIgnoreCase(this.customHeaders(), 'if-match')) {
            return util_1.pickIgnoreCase(this.customHeaders(), 'if-match');
        }
        var eTag = odata_request_traits_1.isWithETag(this.config)
            ? this.config.versionIdentifierIgnored
                ? '*'
                : this.config.eTag
            : undefined;
        return util_1.pickNonNullish({ 'if-match': eTag });
    };
    /**
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    ...
                    case 1: return [2 /*return*/, _a.apply(void 0, _b.concat([(_c.headers = _d.sent(),
                                _c.url = this.relativeUrl(),
                                _c.method = this.config.method,
                                _c.data = this.config.payload,
                                _c)])).catch(function (error) {
    ...
    ODataRequest.prototype.getCsrfHeaders = function (destinationRelatedHeaders) {
        return __awaiter(this, void 0, void 0, function () {
            var customCsrfHeaders;
            return __generator(this, function (_a) {
                customCsrfHeaders = util_1.pickIgnoreCase(this.config.customHeaders, 'x-csrf-token');
                return [2 /*return*/, Object.keys(customCsrfHeaders).length
                        ? customCsrfHeaders
                        : connectivity_1.buildCsrfHeaders(this.destination, {
...
function constructError(error, requestMethod, url) {
    if (error.isAxiosError) {
        var defaultMessage = requestMethod + " request to " + url + " failed!";
        var s4SpecificMessage = util_1.propertyExists(error, 'response', 'data', 'error')
...
function messageFromS4ErrorResponse(error) {
    return (util_1.propertyExists(error.response.data.error, 'message', 'value')
