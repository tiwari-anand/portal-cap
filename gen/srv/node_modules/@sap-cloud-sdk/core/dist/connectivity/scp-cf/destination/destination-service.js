...
function fetchDestinations(destinationServiceUri, jwt, type, options) {
    return __awaiter(this, void 0, void 0, function () {
        var targetUri, destinationsFromCache, headers;
        return __generator(this, function (_a) {
            ...
            if (options === null || options === void 0 ? void 0 : options.useCache) {
                destinationsFromCache = destination_service_cache_1.destinationServiceCache.retrieveDestinationsFromCache(targetUri, jwt_1.decodeJwt(jwt), options.isolationStrategy);
                if (destinationsFromCache) {
                    return [2 /*return*/, destinationsFromCache];
                }
            }
            headers = jwt_1.wrapJwtInHeader(jwt).headers;
            return [2 /*return*/, callDestinationService(targetUri, headers, options)
                    .then(function (response) {
                    var destinations = response.data.map(function (d) {
                        return destination_1.parseDestination(d);
                    });
                    if (options === null || options === void 0 ? void 0 : options.useCache) {
                        destination_service_cache_1.destinationServiceCache.cacheRetrievedDestinations(targetUri, jwt_1.decodeJwt(jwt), destinations, options.isolationStrategy);
                    }
                    return destinations;
                })
 ...
 */
function fetchDestination(destinationServiceUri, token, destinationName, options) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, fetchDestinationByTokens(destinationServiceUri, typeof token === 'string' ? { authHeaderJwt: token } : token, destinationName, options)];
        });
    });
}
exports.fetchDestination = fetchDestination;
function fetchDestinationByTokens(destinationServiceUri, tokens, destinationName, options) {
    return __awaiter(this, void 0, void 0, function () {
        var targetUri, destinationsFromCache, authHeader;
        return __generator(this, function (_a) {
            targetUri = destinationServiceUri.replace(/\/$/, '') + "/destination-configuration/v1/destinations/" + destinationName;
            if (options === null || options === void 0 ? void 0 : options.useCache) {
                destinationsFromCache = destination_service_cache_1.destinationServiceCache.retrieveDestinationsFromCache(targetUri, jwt_1.decodeJwt(tokens.authHeaderJwt), options.isolationStrategy);
                if (destinationsFromCache) {
            ...
            authHeader = jwt_1.wrapJwtInHeader(tokens.authHeaderJwt).headers;
            authHeader = tokens.exchangeHeaderJwt
                ? __assign(__assign({}, authHeader), { 'X-user-token': tokens.exchangeHeaderJwt }) : authHeader;
            return [2 /*return*/, callDestinationService(targetUri, authHeader, options)
                    .then(function (response) {
                    var destination = destination_1.parseDestination(response.data);
                    if (options === null || options === void 0 ? void 0 : options.useCache) {
                        destination_service_cache_1.destinationServiceCache.cacheRetrievedDestinations(targetUri, jwt_1.decodeJwt(tokens.authHeaderJwt), [destination], options.isolationStrategy);
                    }
                    return destination;
                })
                    .catch(function (error) {
                    {
                        throw new util_1.ErrorWithCause("Failed to fetch destination " + destinationName + "." + errorMessageFromResponse(error), error);
                    }
                })];
        });
    });
...
function callDestinationService(uri, headers, options) {
    if (options === void 0) { options = { enableCircuitBreaker: true }; }
    var config = __assign(__assign({}, http_client_1.getAxiosConfigWithDefaults()), { url: uri, headers: headers });
