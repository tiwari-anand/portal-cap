        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [2 /*return*/, util_1.pickNonNullish(__assign(__assign({}, util_1.pickIgnoreCase(csrfHeaders, 'x-csrf-token')), { cookie: buildCookieHeaderValue(util_1.pickValueIgnoreCase(csrfHeaders, 'set-cookie')) }))];
            }
        });
    });
}
...
function makeCsrfRequest(destination, requestConfig) {
    var fetchHeader = !util_1.pickValueIgnoreCase(requestConfig.headers, 'x-csrf-token') && { 'x-csrf-token': 'Fetch' };
    var axiosConfig = __assign(__assign({ method: 'get' }, requestConfig), { headers: __assign(__assign({}, fetchHeader), requestConfig.headers), url: requestConfig.url });
    return http_client_1.executeHttpRequest(destination, axiosConfig)
        .then(function (response) { return response.headers; })
        .catch(function (error) {
        if (!error.response) {
            // TODO: remove once https://github.com/axios/axios/issues/3369 is fixed
            var retry = axiosWorkaround(error, requestConfig, destination);
            if (retry) {
                return retry;
            }
            throw new util_1.ErrorWithCause('Csrf fetch failed.', error);
        }
        return error.response.headers;
    });
}
function axiosWorkaround(error, axiosConfig, destination) {
    var _a, _b, _c;
    if (((_a = error.request) === null || _a === void 0 ? void 0 : _a._isRedirect) && ((_c = (_b = error.request) === null || _b === void 0 ? void 0 : _b._options) === null || _c === void 0 ? void 0 : _c.path)) {
        logger.warn('Csrf fetch was redirected and failed. This might be a bug in the underlying request library (https://github.com/axios/axios/issues/3369).\nRetrying with full configuration.');
        return makeCsrfRequest(destination, __assign(__assign({}, axiosConfig), { url: error.request._options.path }));
    }
}
