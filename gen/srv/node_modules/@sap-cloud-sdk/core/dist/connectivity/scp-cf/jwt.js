...
function verifyJwt(token, options) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            ...
            creds = environment_accessor_1.getXsuaaServiceCredentials(token);
...
function fetchAndCacheKeyAndVerify(creds, verificationKeyURL, token, options) {
    return getVerificationKey(creds, verificationKeyURL)
        ...
        throw new util_1.ErrorWithCause('Failed to verify JWT - unable to get verification key!', error);
    })
        .then(function (key) {
...
function getVerificationKey(xsuaaCredentials, jku) {
    return xsuaa_service_1.fetchVerificationKeys(xsuaaCredentials, jku).then(function (verificationKeys) {
...
function verifyJwtWithKey(token, key) {
    return new Promise(function (resolve, reject) {
        jsonwebtoken_1.default.verify(token, sanitizeVerificationKey(key), function (err, decodedToken) {
            if (err) {
                reject(new util_1.ErrorWithCause('JWT invalid', err));
            }
            else {
...
function isIdenticalTenant(decodedUserToken, decodedProviderToken) {
    return (readPropertyWithWarn(decodedUserToken, tenant_1.mappingTenantFields.id.keyInJwt) ===
        readPropertyWithWarn(decodedProviderToken, tenant_1.mappingTenantFields.id.keyInJwt));
}
...
function customAttributes(decodedToken) {
    if (decodedToken[user_1.mappingUserFields.customAttributes.keyInJwt]) {
        return readPropertyWithWarn(decodedToken, user_1.mappingUserFields.customAttributes.keyInJwt);
    }
    return new Map();
}
