...
function getXsuaaServiceCredentials(token) {
    if (typeof token === 'string') {
        return getXsuaaServiceCredentials(jwt_1.decodeJwt(token)); // Decode without verifying
    }
...
function selectXsuaaInstance(token) {
    ...
    if (selected.length > 1) {
        logger.warn("Multiple XSUAA instances could be matched to the given JWT! Choosing the first one (xsappname: " + util_1.first(selected).credentials.xsappname + ").");
    }
    return util_1.first(selected).credentials;
}
function applyStrategiesInOrder(selectionStrategies, xsuaaInstances, token) {
    return selectionStrategies.reduce(function (result, strategy) {
        return result.length ? result : strategy(xsuaaInstances, token);
    }, []);
}
...
function matchingAudience(xsuaaInstances, token) {
    ...
    return xsuaaInstances.filter(function (xsuaa) {
        return jwt_1.audiences(token).has(xsuaa.credentials.xsappname);
    });
}
