...
exports.getAxiosConfigWithDefaultsWithoutMethod = exports.getAxiosConfigWithDefaults = exports.executeHttpRequest = exports.buildAxiosRequestConfig = exports.execute = exports.addDestinationToRequestConfig = exports.buildHttpRequest = void 0;
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (customHeaders) {
                        logger.warn("The custom headers are provided with the keys: " + Object.keys(customHeaders) + ". These keys will overwrite the headers created by the SDK.");
                    }
                    return [4 /*yield*/, resolveDestination(destination)];
                case 1:
                    resolvedDestination = _a.sent();
                    if (!resolvedDestination) {
                        throw Error("Failed to resolve the destination: " + scp_cf_1.toDestinationNameUrl(destination) + ".");
                    }
...
function execute(executeFn) {
    return function (destination, requestConfig) {
        return __awaiter(this, void 0, void 0, function () {
 ...
 */
exports.executeHttpRequest = execute(executeWithAxios);
function buildDestinationHttpRequestConfig(destination, headers) {
    return __assign({ baseURL: destination.url, headers: headers }, http_agent_1.getAgentConfig(destination));
}
function buildHeaders(destination, customHeaders) {
    return scp_cf_1.buildHeadersForDestination(destination, customHeaders).catch(function (error) {
        throw new util_1.ErrorWithCause('Failed to build HTTP request for destination: failed to build headers!', error);
    });
}
function resolveDestination(destination) {
    return scp_cf_1.useOrFetchDestination(destination).catch(function (error) {
        throw new util_1.ErrorWithCause('Failed to build HTTP request for destination: failed to load destination!', error);
    });
}
...
function executeWithAxios(request) {
    return axios_1.default.request(__assign(__assign({}, getAxiosConfigWithDefaults()), request));
}
/**
