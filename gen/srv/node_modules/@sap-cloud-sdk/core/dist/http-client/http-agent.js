...
function createProxyAgent(destination) {
    ...
    return scp_cf_1.proxyAgent(destination);
}
var trustAllOptions = function (destination) { return function (options) {
    return util_1.assoc('rejectUnauthorized', !destination.isTrustingAllCertificates, options);
}; };
...
function hasSupportedFormat(certificate) {
    var certificateFormat = util_1.last(certificate.name.split('.'));
...
function selectCertificate(destination) {
    ...
    if (!hasSupportedFormat(certificate)) {
        var format = util_1.last(certificate.name.split('.'));
...
function createDefaultAgent(destination) {
    if (scp_cf_1.getProtocolOrDefault(destination) === scp_cf_1.Protocol.HTTPS) {
        if (destination.isTrustingAllCertificates) {
